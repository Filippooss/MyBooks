...
class VerticalScrolledFrame(tk.Frame):
    def __init__(self, master):
        ...
        self.var_message: StringVar = StringVar()

        self.vscrollbar: ttk.Scrollbar = ttk.Scrollbar(self, orient="vertical")
        self.hscrollbar: ttk.Scrollbar = ttk.Scrollbar(self, orient="horizontal")
        self.cv_container: tk.Canvas = tk.Canvas(
            self,
            bd=0,
        ...
        self.f_intirior: tk.Frame = tk.Frame(self.cv_container)  # apalo kokkino
        self.f_message_box: tk.Frame = tk.Frame(self.f_intirior)
        self.lb_message: ttk.Label = ttk.Label(
            self.f_message_box, textvariable=self.var_message
        )
        self.bt_retry: ttk.Button = ttk.Button(self.f_message_box, text="Retry")
        ...
        self.window = self.cv_container.create_window(
            (0, 0), window=self.f_intirior, anchor="nw"
        )
    ...
    def configure_canvas(self, event):
        ...
        if self.f_intirior.winfo_reqwidth() < self.cv_container.winfo_width():
            self.cv_container.itemconfigure(
                self.window, width=self.cv_container.winfo_width()
            )
        else:
            self.cv_container.itemconfigure(
                self.window, width=self.f_intirior.winfo_reqwidth()
            )
    ...
    def show_message(self, message: str, command: Callable[[], Any]):
        ...
        _ = self.bt_retry.config(command=command)
