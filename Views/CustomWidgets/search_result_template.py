...
class SearchResultTemplate(tk.Frame):
    def __init__(
        self, master, book_model: Book, on_event, template_id: int, username: str
    ):
        super().__init__(master=master)
        self.config(bg="black", padx=5, pady=5)

        self.username: str = username
        self.book_model: Book = book_model
        ...
        add_raw = Image.open(
            file_manager.resource_path("Assets/bookmark_add_128_black.png")
        )
        ...
        try:
            ...
            temp = Image.open(
                file_manager.resource_path("Assets/no_image.png")
            )  # fallback image
        temp = temp.resize((135, 200))
        self.image = ImageTk.PhotoImage(temp)

        # create widgets
        self.cv_image = tk.Canvas(
            self,
            width=temp.size[0],
            height=temp.size[1],
            borderwidth=0,
        ...
        self.lb_author = ttk.Label(
            self.f_info,
            text=f"Author: {book_model.author}",
            style="new.TLabel",
            justify="left",
        )
        self.bt_add_book = ttk.Button(
            self.f_info, image=self.tk_image_add, command=self.on_add_book
        )
        self.bt_inspect_book = ttk.Button(
            self.f_info, text="Read More", command=self.on_inspect_book
        )
    ...
    def on_add_book(self):
        add_raw = Image.open(
            file_manager.resource_path("Assets/bookmark_added_128_black.png")
        )
        add_raw = add_raw.resize((20, 20), Image.Resampling.LANCZOS)
        self.tk_image_add = ImageTk.PhotoImage(add_raw)
