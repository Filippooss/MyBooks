...
class LoginView(View):
    def __init__(self, app, view_manager, args):
        ...
        self.var_password: StringVar = tk.StringVar()
        self.var_username: StringVar = tk.StringVar()
        self.var_password_info: StringVar = tk.StringVar()
        self.var_save_credentials: BooleanVar = tk.BooleanVar()

        # define widgets
        self.frame: ttk.Frame = ttk.Frame(self)
        self.lb_title: ttk.Label = ttk.Label(
            self, text="Login", font=("Arial", 40, "bold")
        )
        self.lb_username: ttk.Label = ttk.Label(
            self.frame, text="Enter Username", font=("Arial", 10)
        )
        self.entry_username: ttk.Entry = ttk.Entry(
            self.frame, textvariable=self.var_username
        )
        self.lb_password: ttk.Label = ttk.Label(
            self.frame, text="Enter Password", font=("Arial", 10)
        )
        self.entry_password: ttk.Entry = ttk.Entry(
            self.frame, show="*", textvariable=self.var_password
        )
        self.btn_login: ttk.Button = ttk.Button(
            self.frame, text="Login", command=lambda: self.on_login()
        )
        self.lb_password_info: ttk.Label = ttk.Label(
            self.frame, textvariable=self.var_password_info
        )
        self.cb_save_credentials: ttk.Checkbutton = ttk.Checkbutton(
            self.frame,
            text="Save Credentials",
            variable=self.var_save_credentials,
            command=self.on_save_credentials,
        )
        self.bt_signup: ttk.Button = ttk.Button(
            self.frame, text="Sign Up", command=lambda: self.navigate_to_signup_view()
        )
        self.bt_skip_login: ttk.Button = ttk.Button(
            self, text="Skip Login", command=self.on_skip_login
        )
    ...
    def on_login(self):
        ...
        is_correct: bool = database.login_user(
        ...
        if is_correct:
            ...
            self.app.set_current_user(
                User(self.var_username.get(), self.var_password.get())
            )
    ...
    def on_save_credentials(self):
        save_data = {"save_credentials": self.var_save_credentials.get()}
