...
class SignupView(View):
    def __init__(self, app: tk.Misc, view_manager, args):
        ...
        self.var_username: StringVar = StringVar()
        self.var_password: StringVar = StringVar()
        self.var_repassword: StringVar = StringVar()
        self.var_password_info: StringVar = StringVar()
        self.var_repassword_info: StringVar = StringVar()

        # define widgets
        self.frame: Frame = ttk.Frame(self)
        self.title_label: Label = ttk.Label(
            self, text="Singup", font=("Arial", 40, "bold")
        )
        self.ewt_username: EntryWithText = EntryWithText(
            self.frame, "Enter Username", var_entry=self.var_username
        )
        self.ewt_password: EntryWithText = EntryWithText(
            self.frame, "Enter Password", var_entry=self.var_password
        )
        self.ewt_repassword: EntryWithText = EntryWithText(
            self.frame, "Confirm Password", var_entry=self.var_repassword
        )
        self.bt_signup: Button = ttk.Button(
            self.frame, text="Sign up", command=lambda: self.on_sign_up()
        )
        self.bt_login: Button = ttk.Button(
            self.frame, text="Already have an account?", command=lambda: self.on_login()
        )
        self.lb_password_info: Label = ttk.Label(
            self.frame, textvariable=self.var_password_info, foreground="red"
        )
        self.lb_repassword_info: Label = ttk.Label(
            self.frame, textvariable=self.var_repassword_info, foreground="red"
        )

        self.var_password.trace_add("write", callback=self.on_password_change_callback)
        self.var_repassword.trace_add(
            "write", callback=self.on_repassword_change_callback
        )
    ...
    def on_sign_up(self):
        if (
            self.ewt_password.get_value() == self.ewt_repassword.get_value()
            and self.is_password_valid == True
            and self.is_username_valid == True
        ):

            database.insert_user(
                self.ewt_username.get_value(), self.ewt_password.get_value()
            )
            self._view_manager.change_view("LoginView")
        else:
            print("Cant sign up")
